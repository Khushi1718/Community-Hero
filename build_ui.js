...
// 1. Replace standard tabs with standard official tabs
const newTabs = `
        {/* Tabs */}
...
// 2. Update Header to remove shield and gradients
const newHeader = `
...
// 3. Clean up Admin Table styling
...
// User asked: "make greay lines of table sin ui light green and make ui nice"
content = content.replace(/border-slate-200/g, "border-emerald-100");
content = content.replace(/border-slate-300/g, "border-emerald-200");

// 4. Redesign Area Adoptions
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {admins.filter(a => a.city === "ALL" && a.department === "ALL").length === 0 ? (
                  ...
                  <tr key={admin.id} className="border-b border-emerald-100 hover:bg-emerald-50/30 transition-colors">
                    <td className="p-4">
...
// 5. Replace AI Report (the first one) and insert Audit Logs next to it
const newTabsContent = `
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {auditLogs.length === 0 ? (
                  ...
                  <tr key={log._id} className="border-b border-emerald-100 hover:bg-emerald-50/30 text-xs">
      ...
      {/* AI Report Tab */}
      {activeTab === "ai_report" && (
        <div className="bg-white border border-emerald-200 rounded-sm shadow-sm overflow-hidden p-6">
          ...
          {aiReportData ? (
             ...
             <div className="text-center py-12 text-slate-500 text-sm border-2 border-dashed border-emerald-200 rounded">
...
// Replace the FIRST AI Report tab
content = content.replace(/{\/\* AI Report Tab \*\/}.*?(?=\n\s*<\/div>\n\s*<\/div>\n\s*{\/\* Admin Modal \*\/})/s, newTabsContent);

// Remove the duplicate mock AI report tab located around line 800 before {/* Add Organization Modal */}
// We have to be very careful to only remove the AI Report Tab block!
content = content.replace(/{\/\* AI Report Tab \*\/}.*?(?=\n\s*{\/\* Add Organization Modal \*\/})/s, "");

// Add light green background: replace bg-slate-50 with bg-[#F4F9F5] on the main container
