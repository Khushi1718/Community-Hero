...
import {
  ...
  Calendar, PieChart, BarChart2, Award, Download
} from "lucide-react";
...
export default function AdminPage() {
  ...
  if (role !== "admin") {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F4F9F5]">
        <div className="text-center p-8 bg-white rounded-none border border-red-200 shadow-sm max-w-sm">
          <AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4" />
          <h2 className="text-xl font-bold text-slate-900 mb-2">Access Denied</h2>
          <p className="text-slate-500 mb-4">You don't have admin privileges.</p>
          <button onClick={() => router.push("/login")} className="bg-slate-900 text-white px-6 py-2 rounded-none font-bold">Go to Login</button>
        </div>
      </div>
    );
  }
  ...
  const IssueCard = ({ issue, showActions = true }: { issue: Issue; showActions?: boolean }) => {
    ...
    return (
      <div className={`bg-white border rounded-none overflow-hidden transition-all shadow-sm hover:shadow-md ${isEscalated ? "border-orange-300" : "border-emerald-100"}`}>
        {/* Card Header */}
        <div
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-4 flex-1 min-w-0">
              {issue.imageBase64 && (
                <img src={issue.imageBase64} alt="" className="w-16 h-16 rounded-none object-cover border border-emerald-100 flex-shrink-0" />
              )}
        ...
        {/* Expanded Content */}
        {isExpanded && (
          <div className="border-t border-slate-100 bg-[#F4F9F5] p-5 space-y-5">
            
            {/* Verification Metadata */}
            {(issue.verificationScore !== undefined || issue.cameraSource) && (
              <div className="bg-white border border-emerald-100 rounded-none p-4 shadow-sm">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {issue.verificationScore !== undefined && (
                    <div className="bg-[#F4F9F5] p-3 rounded-none border border-slate-100">
                  ...
                  {issue.deviceInfo && (
                    <div className="bg-[#F4F9F5] p-3 rounded-none border border-slate-100 col-span-2">
            ...
            {issue.aiAnalysis?.severityReason && (
              <div className="bg-red-50/50 border border-red-100 rounded-none p-4 shadow-sm">
                <h4 className="text-sm font-bold text-red-800 mb-2 flex items-center gap-2">
                  <AlertTriangle className="w-4 h-4" /> AI Severity Analysis: {issue.aiAnalysis.severity}
                </h4>
                <p className="text-sm text-red-900 font-medium bg-white p-3 rounded-none border border-red-100 shadow-inner-soft">
            ...
            {issue.resolutionProof && (
              <div className="bg-green-50 border border-green-200 rounded-none p-4">
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  {issue.imageBase64 && <div><p className="text-xs text-slate-500 mb-1">Before</p><img src={issue.imageBase64} className="w-full h-28 object-cover rounded-none border" alt="before" /></div>}
                  <div><p className="text-xs text-green-600 font-bold mb-1">After (Evidence)</p><img src={issue.resolutionProof.imageBase64} className="w-full h-28 object-cover rounded-none border border-green-300" alt="after" /></div>
                </div>
                <p className="text-sm text-slate-700 bg-white p-3 rounded-none border">{issue.resolutionProof.notes}</p>
            ...
            {showActions && !["Closed", "Resolved", "Completed", "Rejected"].includes(issue.status) && (
              <div className="flex flex-wrap gap-3">
                  ...
                  assigningIssueId === issue.id ? (
                    <div className="flex gap-2 items-center">
                      <select
                        ...
                        className="border border-emerald-100 rounded-none px-3 py-2 text-sm bg-white focus:ring-2 focus:ring-purple-500 focus:outline-none"
                      >
                ...
                {resolvingIssueId === issue.id ? (
                  <div className="w-full bg-white border border-emerald-100 rounded-none p-4 space-y-3">
                    ...
                    {resolutionImageBase64 && <img src={resolutionImageBase64} className="h-24 rounded-none object-cover" alt="preview" />}
                    <textarea value={resolutionNotes} onChange={e => setResolutionNotes(e.target.value)} placeholder="Resolution notes…" className="w-full border border-emerald-100 rounded-none p-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-purple-500" rows={3} />
                    <div className="flex gap-2">
                      <button onClick={() => submitResolution(issue.id)} className="bg-green-600 text-white px-4 py-2 rounded-none text-sm font-bold hover:bg-green-700">Submit</button>
                      <button onClick={() => setResolvingIssueId(null)} className="text-slate-500 px-4 py-2 rounded-none text-sm hover:bg-slate-100">Cancel</button>
                    </div>
                  </div>
                ) : (
  ...
  return (
    <div className="flex h-[100dvh] bg-[#F4F9F5] font-sans overflow-hidden">
      ...
      {/* ─── SIDEBAR ─── */}
      <aside className="w-64 bg-emerald-900 text-white flex flex-col shadow-sm border-r border-emerald-800 flex-shrink-0 z-20">
        <div className="p-6 border-b border-surface-800">
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              <h1 className="text-xl font-black text-white tracking-tight leading-tight"><span className="text-emerald-400">STATE</span> ADMIN PORTAL</h1>
          ...
          {NAV_ITEMS.map(item => (
            ...
            >
              ...
              {item.badge !== undefined && item.badge > 0 && (
                <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${activeTab === item.id ? "bg-white/20 text-white" : "bg-emerald-900 text-surface-400"}`}>
                  {item.badge}
                </span>
              )}
            </button>
        ...
        <div className="p-5 border-t border-surface-800 space-y-3">
          ...
          <button onClick={handleToggleAdminStatus} className={`w-full flex items-center gap-2 justify-center py-3 rounded-none text-sm font-bold transition-all border-2 ${isOnDuty ? "border-success-500/30 bg-success-500/10 text-success-400 hover:bg-success-500/20" : "border-surface-700 bg-emerald-900 text-surface-400 hover:bg-surface-700"}`}>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded-sm p-4 flex items-start gap-3 shadow-sm">
                 ...
                 <div>
                    ...
                    <p className="text-xs text-emerald-700 mt-1">All core municipal services in your jurisdiction are operating within normal parameters. {liveIssues.length} issues require attention.</p>
                 </div>
              </div>
                        ...
                        <div key={issue.id} className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 hover:bg-emerald-50/30 transition-colors cursor-pointer" onClick={() => { setActiveTab("live"); setExpandedIssueId(issue.id); }}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded ${issue.status === "Resolved" ? "bg-emerald-100 text-emerald-700" : issue.status === "In Progress" || issue.status === "Work In Progress" ? "bg-amber-100 text-amber-700" : "bg-blue-100 text-blue-700"}`}>{issue.status}</span>
                            </div>
                            <h4 className="font-bold text-slate-900 text-sm truncate">{issue.aiAnalysis?.category || "Issue Report"}</h4>
                          </div>
                  ...
                  <div className="bg-white border border-slate-200 rounded-sm shadow-sm overflow-hidden">
                    ...
                    <div className="p-4 space-y-4">
                      {employees.slice(0, 4).map(emp => (
                        <div key={emp.email} className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1.5">
                            <div className={`w-2 h-2 rounded-full ${emp.isAvailable !== false ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-slate-300'}`}></div>
                            <span className="text-[10px] font-bold text-slate-500 uppercase">{emp.isAvailable !== false ? 'Active' : 'Offline'}</span>
                          </div>
                        </div>
                      ))}
                      {employees.length === 0 && (
          ...
          {activeTab === "live" && (
            <div className="space-y-6">
              ...
              {filteredLive.length === 0 ? (
                <Card className="border-emerald-100 p-16 text-center">
                ...
                {materialIssues.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {materialIssues.map(issue => (
                      <Card key={issue.id} className="border-emerald-100 overflow-hidden">
                        <div className="flex justify-between items-start p-6 bg-[#F4F9F5] border-b border-emerald-100">
                          <div>
                            <span className="font-mono text-[10px] font-bold text-surface-500 bg-white border border-emerald-100 shadow-sm px-2 py-1 rounded-md mr-3">{issue.id}</span>
                        ...
                        <CardContent className="p-6">
                          <div className="grid md:grid-cols-2 gap-6">
                            <div className="bg-warning-50 border border-warning-200 rounded-none p-5 shadow-sm">
                ...
                {verifyIssues.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {verifyIssues.map(issue => (
                      <Card key={issue.id} className="border-emerald-100 overflow-hidden">
                        <div className="p-6 border-b border-emerald-100 flex justify-between items-start bg-[#F4F9F5]/50">
                          <div>
                            <span className="font-mono text-[10px] font-bold text-surface-500 bg-white border border-emerald-100 shadow-sm px-2 py-1 rounded-md mr-3">{issue.id}</span>
                            <h3 className="font-black text-surface-900 text-xl inline">{issue.aiAnalysis.category}</h3>
                          ...
                          <div className="flex gap-3 w-full md:w-auto">
                            {overrideIssueId === issue.id ? (
                              <div className="flex gap-2 items-center flex-1 w-full bg-white p-2 rounded-none border border-red-200 shadow-sm">
                        ...
                        <CardContent className="p-6 grid md:grid-cols-2 gap-8">
                          <div>
                            ...
                            {issue.imageBase64 && <img src={issue.imageBase64} className="w-full h-56 object-cover rounded-none border-2 border-emerald-100 shadow-sm" alt="Original" />}
                            <div className="mt-4 bg-[#F4F9F5] rounded-none p-4 border border-emerald-100 shadow-inner-soft">
                          ...
                          <div>
                            ...
                            {(issue.evidences && issue.evidences.length > 0) ? (
                              <>
                                <img src={issue.evidences[issue.evidences.length - 1].url} className="w-full h-56 object-cover rounded-none border-4 border-success-400 shadow-sm" alt="Resolution" />
                                <div className="mt-4 bg-success-50 rounded-none p-4 border border-success-200 shadow-inner-soft text-sm text-success-900">
                              ...
                              <>
                                <img src={issue.resolutionProof.imageBase64} className="w-full h-56 object-cover rounded-none border-4 border-success-400 shadow-sm" alt="Resolution" />
                                <div className="mt-4 bg-success-50 rounded-none p-4 border border-success-200 shadow-inner-soft text-sm text-success-900">
                                  <span className="font-black uppercase tracking-wider text-[10px] block mb-1 text-success-700">Employee Notes:</span>
                              ...
                              <div className="h-56 border-2 border-dashed border-surface-300 rounded-none flex items-center justify-center text-surface-400 font-medium">No proof submitted</div>
                            )}
                          </div>
                        </CardContent>
                        {issue.resolutionVerification && (
                          <div className="bg-indigo-50/50 border-t border-b border-indigo-100 p-6">
                            ...
                            <div className="flex flex-col md:flex-row gap-4 items-start">
                              <div className={`flex flex-col items-center justify-center p-4 rounded-none border-2 w-32 shrink-0 ${issue.resolutionVerification.isResolved ? "bg-success-50 border-success-200 text-success-700" : "bg-red-50 border-red-200 text-red-700"}`}>
                              ...
                              <div className="bg-white p-4 rounded-none border border-indigo-100 shadow-sm flex-1">
                ...
                <Card className="border-emerald-100">
                  ...
                  {employees.length === 0 ? (
                    ...
                    <div className="divide-y divide-surface-100">
                      {employees.map(emp => (
                        <div key={emp.email} className="flex flex-col sm:flex-row sm:items-center gap-4 p-6 hover:bg-[#F4F9F5] transition-colors">
                          <div className="w-12 h-12 bg-gradient-to-br from-primary-500 to-info-600 text-white rounded-none flex items-center justify-center font-black text-lg shadow-inner-soft flex-shrink-0">
                            {emp.name.charAt(0).toUpperCase()}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="inline-block px-2.5 py-1 bg-emerald-50 text-surface-600 text-[10px] font-bold uppercase tracking-wider rounded-md">{emp.department || appUser.department}</span>
                          </div>
                          <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 mt-2 sm:mt-0">
                            <span className={`px-3 py-1.5 rounded-full text-xs font-black uppercase tracking-wider ${emp.isAvailable !== false ? "bg-success-100 text-success-700 border border-success-200" : "bg-emerald-50 text-surface-500 border border-emerald-100"}`}>
              ...
              <div className="space-y-6">
                ...
                <Card className="border-emerald-100">
                  ...
                  <CardContent className="space-y-3 text-sm">
                    <div className="flex justify-between items-center py-2 border-b border-emerald-100"><span className="text-surface-500 font-bold uppercase tracking-wider text-[10px]">State</span><span className="font-black text-surface-900">{appUser.state || "—"}</span></div>
                    <div className="flex justify-between items-center py-2 border-b border-emerald-100"><span className="text-surface-500 font-bold uppercase tracking-wider text-[10px]">City</span><span className="font-black text-surface-900">{appUser.city || "Entire State"}</span></div>
                  ...
                  <CardContent className="pt-5">
                    <form onSubmit={handleCreateEmployee} className="space-y-4">
                      <div>
                        ...
                        <input type="text" required value={newEmpName} onChange={e => setNewEmpName(e.target.value)} placeholder="e.g. John Doe" className="w-full border-2 border-emerald-100 bg-white rounded-none px-4 py-3 text-sm focus:border-primary-500 focus:outline-none transition-colors placeholder:text-surface-400" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-surface-600 uppercase tracking-wider mb-1.5">Login Email</label>
                        <input type="email" required value={newEmpEmail} onChange={e => setNewEmpEmail(e.target.value)} placeholder="employee@example.com" className="w-full border-2 border-emerald-100 bg-white rounded-none px-4 py-3 text-sm focus:border-primary-500 focus:outline-none transition-colors placeholder:text-surface-400" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-surface-600 uppercase tracking-wider mb-1.5">Initial Password</label>
                        <input type="text" required value={newEmpPassword} onChange={e => setNewEmpPassword(e.target.value)} placeholder="Set a secure password" className="w-full border-2 border-emerald-100 bg-white rounded-none px-4 py-3 text-sm focus:border-primary-500 focus:outline-none transition-colors placeholder:text-surface-400" />
                      </div>
                      {empFormMsg && (
                        <div className={`text-xs font-bold px-4 py-3 rounded-none border ${empFormMsg.type === "success" ? "bg-success-50 text-success-700 border-success-200" : "bg-error-50 text-error-700 border-error-200"}`}>
                          {empFormMsg.text}
                        </div>
                      )}
          ...
          {activeTab === "volunteer_orgs" && (() => {
            ...
            return (
              <div className="space-y-6">
                ...
                <div className="flex gap-2 flex-wrap mb-4">
                  {(["pending", "verified", "rejected", "create"] as const).map(tab => (
                    <button
                      ...
                      className={`px-5 py-2.5 rounded-none text-sm font-bold transition-all capitalize ${
                        orgSubTab === tab
                          ? "bg-primary-600 text-white"
                          : "bg-white border border-emerald-100 text-surface-600 hover:bg-[#F4F9F5]"
                      }`}
                    >
                ...
                {orgSubTab === "create" ? (
                   <Card className="border-emerald-100 shadow-sm max-w-2xl">
                     <CardHeader className="border-b border-emerald-100 bg-[#F4F9F5] p-6">
                       ...
                       <form onSubmit={async (e) => {
                         ...
                         <div className="grid grid-cols-2 gap-4">
                           <div>
                             ...
                             <input name="name" required className="w-full border p-2 rounded-none text-sm" />
                           </div>
                           <div>
                             <label className="block text-xs font-bold text-surface-600 mb-1">Type</label>
                             <select name="type" className="w-full border p-2 rounded-none text-sm">
                           ...
                           <div className="col-span-2">
                             ...
                             <textarea name="description" required rows={2} className="w-full border p-2 rounded-none text-sm resize-none"></textarea>
                           </div>
                           <div>
                             <label className="block text-xs font-bold text-surface-600 mb-1">Contact Email (Login ID)</label>
                             <input name="contactEmail" type="email" required className="w-full border p-2 rounded-none text-sm" />
                           </div>
                           <div>
                             <label className="block text-xs font-bold text-surface-600 mb-1">Contact Phone</label>
                             <input name="contactPhone" required className="w-full border p-2 rounded-none text-sm" />
                           </div>
                           <div>
                             <label className="block text-xs font-bold text-surface-600 mb-1">Contact Person Name</label>
                             <input name="contactPersonName" required className="w-full border p-2 rounded-none text-sm" />
                           </div>
                           <div>
                             <label className="block text-xs font-bold text-surface-600 mb-1">Primary Category</label>
                             <select name="workCategory" className="w-full border p-2 rounded-none text-sm">
                           ...
                           <div className="col-span-2">
                             ...
                             <input name="address" required className="w-full border p-2 rounded-none text-sm" />
                           </div>
                           {!appUser.city && (
                             <>
                               <div><label className="block text-xs font-bold text-surface-600 mb-1">City</label><input name="city" required className="w-full border p-2 rounded-none text-sm" /></div>
                               <div><label className="block text-xs font-bold text-surface-600 mb-1">State</label><input name="state" required className="w-full border p-2 rounded-none text-sm" /></div>
                             </>
                           )}
                           <div>
                             <label className="block text-xs font-bold text-surface-600 mb-1">Active Members</label>
                             <input name="activeMembers" type="number" required defaultValue="10" className="w-full border p-2 rounded-none text-sm" />
                           </div>
                         </div>
                  ...
                  <div className="space-y-4">
                    {displayOrgs.map((org: any) => (
                      <div key={org._id} className="bg-white border border-emerald-100 rounded-none overflow-hidden shadow-sm">
                        <div className="p-5 flex flex-col sm:flex-row sm:items-start gap-4">
                          {/* Logo */}
                          <div className="w-16 h-16 rounded-none overflow-hidden bg-emerald-100 flex items-center justify-center flex-shrink-0 border border-emerald-100">
                            {org.logoUrl
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-1">
                              ...
                              <span className={`text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-wide ${
                                ...
                                org.status === "SUSPENDED" ? "bg-emerald-50 text-surface-600" :
                        ...
                        {/* Actions */}
                        <div className="border-t border-emerald-100 bg-[#F4F9F5]/50 px-5 py-3 flex flex-wrap gap-2">
                          {org.status === "PENDING_VERIFICATION" && (
                            <>
                              <button
                                onClick={() => handleOrgAction(org._id, "approved")}
                                disabled={orgActionLoading}
                                className="flex items-center gap-1.5 bg-success-600 hover:bg-success-700 text-white text-xs font-bold px-4 py-2 rounded-none transition-colors disabled:opacity-50"
                              >
                                <CheckCircle2 className="w-3.5 h-3.5" /> Approve
                              </button>
                              <button
                                onClick={() => { setOrgActionId(org._id); setOrgActionType("reject"); setOrgActionMessage(""); }}
                                className="flex items-center gap-1.5 bg-error-50 hover:bg-error-100 text-error-700 border border-error-200 text-xs font-bold px-4 py-2 rounded-none transition-colors"
                              >
                                <X className="w-3.5 h-3.5" /> Reject
                              </button>
                              <button
                                onClick={() => { setOrgActionId(org._id); setOrgActionType("info"); setOrgActionMessage(""); }}
                                className="flex items-center gap-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 text-xs font-bold px-4 py-2 rounded-none transition-colors"
                              >
                          ...
                          {org.status === "VERIFIED" && (
                            <button
                              ...
                              disabled={orgActionLoading}
                              className="flex items-center gap-1.5 bg-warning-50 hover:bg-warning-100 text-warning-700 border border-warning-200 text-xs font-bold px-4 py-2 rounded-none transition-colors disabled:opacity-50"
                            >
                          ...
                          {org.status === "SUSPENDED" && (
                            <button
                              ...
                              disabled={orgActionLoading}
                              className="flex items-center gap-1.5 bg-success-50 hover:bg-success-100 text-success-700 border border-success-200 text-xs font-bold px-4 py-2 rounded-none transition-colors disabled:opacity-50"
                            >
                          ...
                          {orgActionId === org._id && (orgActionType === "reject" || orgActionType === "info") && (
                            <div className="w-full mt-2 space-y-2">
                              <textarea
                                ...
                                className="w-full border border-emerald-100 rounded-none px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 focus:outline-none resize-none"
                                rows={2}
                              />
                              <div className="flex gap-2">
                                <button
                                  ...
                                  className="bg-emerald-900 text-white text-xs font-bold px-4 py-2 rounded-none disabled:opacity-50"
                                >
                                  {orgActionLoading ? "Sending..." : "Send"}
                                </button>
                                <button onClick={() => { setOrgActionId(null); setOrgActionType(null); }} className="text-surface-500 text-xs px-3 py-2 rounded-none hover:bg-surface-200">
                                  Cancel
                                </button>
                              </div>
                            </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
                ...
                <div className="relative w-full sm:w-auto">
                  ...
                  <input
                    ...
                    className="w-full sm:w-80 pl-10 pr-4 py-2 bg-white border border-emerald-100 rounded-none text-sm focus:ring-2 focus:ring-primary-500 focus:outline-none"
                  />
                </div>
              </div>

              <div className="bg-white border border-emerald-100 rounded-none overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm whitespace-nowrap">
                    <thead className="bg-[#F4F9F5] border-b border-emerald-100 text-surface-500 font-bold uppercase tracking-wider text-[10px]">
                      <tr>
                    ...
                    <tbody className="divide-y divide-surface-100">
                      {certificates
                        ...
                        .map(cert => (
                        <tr key={cert.certificateId} className="hover:bg-[#F4F9F5]/50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              {cert.certificatePdfUrl && (
                                <>
                                  <a href={cert.certificatePdfUrl} download className="p-2 text-surface-400 hover:text-primary-600 hover:bg-primary-50 rounded-none transition-colors" title="Download PDF"><Download className="w-4 h-4"/></a>
                                  <a href={cert.certificateImageUrl} download className="p-2 text-surface-400 hover:text-info-600 hover:bg-info-50 rounded-none transition-colors" title="Download Image"><Eye className="w-4 h-4"/></a>
                                </>
                              )}
                              <button 
                                onClick={async () => {
                                ...
                                className="p-2 text-surface-400 hover:text-warning-600 hover:bg-warning-50 rounded-none transition-colors" title="Resend Email"
                              >
               ...
               <div className="flex gap-2 flex-wrap mb-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {drives.filter(d => {
                    ...
                    <Card key={drive._id} className={`overflow-hidden border-2 ${drive.status === "OVERDUE" ? "border-error-300 bg-error-50" : "border-emerald-100"}`}>
                      <CardContent className="p-5">
                        ...
                        {drive.status === "ORG_PENDING_APPROVAL" && drive.orgRequests?.filter((r:any) => r.status === "pending").map((req: any) => (
                           <div key={req.orgId} className="bg-white border border-emerald-100 p-3 rounded-none mb-2">
                             ...
                             <div className="flex gap-2">
                               ...
                               }} className="flex-1 bg-success-600 hover:bg-success-700 text-white text-xs font-bold py-2 rounded-none transition-colors">Approve</button>
                               <button onClick={() => {
                               ...
                               }} className="flex-1 bg-error-50 hover:bg-error-100 text-error-700 text-xs font-bold py-2 rounded-none transition-colors">Reject</button>
                             </div>
                           </div>
                        ))}

                        {drive.status === "OVERDUE" && (
                           <div className="bg-white border border-error-200 p-3 rounded-none mt-3">
                             ...
                             <div className="flex gap-2">
                               ...
                               }} className="flex-1 bg-error-600 hover:bg-error-700 text-white text-xs font-bold py-2 rounded-none">Reset to Waiting</button>
                               <button onClick={() => {
                               ...
                               }} className="flex-1 bg-surface-200 hover:bg-surface-300 text-surface-800 text-xs font-bold py-2 rounded-none">Fail & Restore Emp</button>
                             </div>
                           </div>
                        )}

                        {drive.cancellationRequestedBy && !drive.cancellationApprovedAt && (
                           <div className="bg-orange-50 border border-orange-200 p-3 rounded-none mt-3">
                             ...
                             }} className="w-full bg-orange-600 hover:bg-orange-700 text-white text-xs font-bold py-2 rounded-none transition-colors">Approve Cancellation</button>
                           </div>
                        )}

                        {drive.status === "ADMIN_VERIFICATION_PENDING" && (
                           <div className="bg-success-50 border border-success-200 p-4 rounded-none mt-3">
                             ...
                             <div className="grid grid-cols-2 gap-4 mb-3">
                                <div>
                                   ...
                                   <p className="text-xs text-success-900 bg-success-100 p-2 rounded-none">{drive.workPerformed}</p>
                                </div>
                                <div className="grid grid-cols-2 gap-2">
                                   <div className="bg-success-100 p-2 rounded-none">
                                ...
                                <div className="mb-3">
                                  ...
                                  <div className="flex gap-2 overflow-x-auto pb-2">
                                    {drive.afterImageUrls.map((url: string, i: number) => (
                                      <img key={i} src={url} alt={`evidence-${i}`} className="w-24 h-24 object-cover rounded-none flex-shrink-0 border border-success-200" />
                                    ))}
                                  </div>
                                </div>
                             )}
                             ...
                             }} className="w-full bg-success-600 hover:bg-success-700 text-white text-sm font-bold py-2.5 rounded-none transition-colors shadow-sm">
                  ...
                  {drives.filter(d => {
                    ...
                    return false;
                     ...
                     <div className="col-span-1 lg:col-span-2 text-center py-12 bg-white rounded-none border border-emerald-100">
      ...
      {activeTab === "analytics" && (
         <div className="space-y-6 animate-fade-in pb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
               <div className="bg-white p-6 rounded-none border border-emerald-100 shadow-sm flex items-center gap-4">
                  <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-none flex items-center justify-center shrink-0"><Users className="w-6 h-6"/></div>
                  <div>
               ...
               <div className="bg-white p-6 rounded-none border border-emerald-100 shadow-sm flex items-center gap-4">
                  <div className="w-12 h-12 bg-indigo-50 text-indigo-600 rounded-none flex items-center justify-center shrink-0"><Shield className="w-6 h-6"/></div>
                  <div>
               ...
               <div className="bg-white p-6 rounded-none border border-emerald-100 shadow-sm flex items-center gap-4">
                  <div className="w-12 h-12 bg-amber-50 text-amber-600 rounded-none flex items-center justify-center shrink-0"><Award className="w-6 h-6"/></div>
                  <div>
               ...
               <div className="bg-white p-6 rounded-none border border-emerald-100 shadow-sm flex items-center gap-4">
                  <div className="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-none flex items-center justify-center shrink-0"><Activity className="w-6 h-6"/></div>
                  <div>
            ...
            <div className="bg-white rounded-none border border-emerald-100 shadow-sm p-6 lg:p-8">
      ...
      {activeTab === "leaderboard" && (
         <div className="space-y-6 animate-fade-in pb-12">
            ...
            <div className="bg-white rounded-none border border-emerald-100 shadow-sm overflow-hidden">
               <div className="divide-y divide-slate-100">
                  {leaderboard.length > 0 ? leaderboard.map((item, index) => (
                     <div key={item.id} className="p-4 sm:p-6 flex items-center gap-4 hover:bg-[#F4F9F5] transition-colors">
             ...
             <form onSubmit={async (e) => {
               ...
               <div className="grid grid-cols-2 gap-4">
                 <div className="col-span-2">
                   ...
                   <input required value={driveTitle} onChange={e=>setDriveTitle(e.target.value)} className="w-full border p-2.5 rounded-none text-sm" placeholder="e.g. Weekend Lake Cleanup" />
                 </div>
                 <div className="col-span-2">
                   <label className="block text-xs font-bold text-surface-600 mb-1">Description & Goal</label>
                   <textarea required rows={3} value={driveDescription} onChange={e=>setDriveDescription(e.target.value)} className="w-full border p-2.5 rounded-none text-sm resize-none" />
                 </div>
                 <div>
                   <label className="block text-xs font-bold text-surface-600 mb-1">Drive Category</label>
                   <select required value={driveCategory} onChange={e=>setDriveCategory(e.target.value)} className="w-full border p-2.5 rounded-none text-sm">
                 ...
                 <div>
                   ...
                   <select required value={driveReqOrgCat} onChange={e=>setDriveReqOrgCat(e.target.value)} className="w-full border p-2.5 rounded-none text-sm">
                     <option>Cleanliness</option><option>Tree Plantation</option><option>Park Cleaning</option><option>Awareness Campaign</option>
                   </select>
                 </div>
                 <div className="col-span-2">
               ...
               <div className="pt-4 border-t border-emerald-100 flex gap-3 justify-end">
                 <button type="button" onClick={() => setConvertingIssue(null)} className="px-5 py-2.5 text-sm font-bold text-surface-600 hover:bg-emerald-50 rounded-none transition-colors">Cancel</button>
                 <button type="submit" disabled={convertingLoading} className="px-5 py-2.5 text-sm font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-none transition-colors flex items-center gap-2">
        ...
        <div className="space-y-6 animate-fade-in pb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {adoptedAreas.map(area => (
              <Card key={area._id} className="border-emerald-100 shadow-sm">
                  ...
                  {area.status === "PENDING" && (
                    <div className="flex gap-2 pt-2 border-t border-slate-100">
                      <button onClick={async () => {
                        ...
                        loadExtraData();
                      }} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 rounded-none text-sm transition-colors">
                        Approve
                      </button>
                      <button onClick={async () => {
                        ...
                        loadExtraData();
                      }} className="flex-1 bg-red-50 hover:bg-red-100 text-red-700 font-bold py-2 rounded-none text-sm transition-colors border border-red-200">
                        Reject
                      </button>
                    </div>
                  )}
            ...
            {adoptedAreas.length === 0 && (
              <div className="col-span-2 text-center py-12 bg-white rounded-none border border-emerald-100 text-slate-500 font-medium">
      ...
      {activeTab === "health" && (
        <div className="space-y-6 animate-fade-in pb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-none border border-emerald-100 shadow-sm flex items-center justify-between">
              <div>
        ...
        <div className="space-y-6 animate-fade-in pb-12">
          ...
          <div className="bg-white rounded-none border border-emerald-100 shadow-sm overflow-hidden">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-[#F4F9F5] border-b border-emerald-100 text-xs uppercase text-slate-500 font-bold">
              ...
              <tbody className="divide-y divide-slate-100">
                {auditLogs.map((log: any) => (
                  <tr key={log._id} className="hover:bg-[#F4F9F5] transition-colors">
      ...
      {activeTab === "heatmap" && (
        <div className="space-y-6 animate-fade-in pb-12 h-full flex flex-col">
          ...
          <div className="flex-1 bg-slate-200 rounded-none border border-slate-300 overflow-hidden relative min-h-[500px]">
